...
/**
 * The TextIcon will paint a String of text as an Icon. The Icon can be used by
 * any Swing component that supports icons.
 * 
 ...
 * </ul>
 * 
 * TextIcon was designed to be rendered on a specific JComponent as it requires
 * FontMetrics information in order to calculate its size and to do the
 * rendering. Therefore, it should only be added to component it was created
 * for.
 * 
 * By default the text will be rendered using the Font and foreground color of
 * its associated component. However, this class does allow you to override
 * these properties.
 */
public class TextIcon implements Icon, PropertyChangeListener {
	 ...
	 * 
	 * @param component
	 *            the component to which the icon will be added
	 * @param text
	 ...
	 *            the text to be rendered on the Icon
	 * @param layout
	 *            specify the layout of the text. Must be one of the Layout
	 *            enums: HORIZONTAL or VERTICAL
	 */
	...
	/**
	 * Get the Font used to render the text. This will default to the Font of
	 * the component unless the Font has been overridden by using the setFont()
	 * method.
	 * 
	 ...
	 * text. You can specify some padding to be added to the start and end of
	 * the text when it is rendered.
	 * 
	 * @param padding
	 ...
	 * 
	 * @param c
	 *            The component to which the icon is added
	 * @param g
	...
	public void paintIcon(Component c, Graphics g, int x, int y) {
		...
		if (layout == Layout.HORIZONTAL) {
			...
			for (int i = 0; i < text.length(); i++) {
				int offsetX = Math
						.round((getIconWidth() - stringWidths[i]) / 2.0f);
